...
- Create file `gradle.properties` in `easy-repo-local` directory with following content
    ...
    ```
 - Alternatively, you can create your repository using [Easy Repository Template](https://github.com/apanwar/easy-repository-template) on GitHub by clicking [Here](https://github.com/new?template_name=easy-repository-template&template_owner=apanwar&name=easy-repo-local&description=easy-repo-local) and updating the `rootProject.name` property in `settings.gradle` and `sap.commerce.easy.repository.code` property in `gradle.properties`
